...
export function FlowButton({ text = 'Modern Button', onClick, className = '' }: FlowButtonProps) {
  return (
    <button
      onClick={onClick}
      className={`group relative flex items-center gap-1 overflow-hidden rounded-[100px] border-[1.5px] border-body/40 bg-transparent px-8 py-3 text-sm font-semibold text-dark cursor-pointer transition-all duration-[600ms] ease-[cubic-bezier(0.23,1,0.32,1)] hover:border-transparent hover:text-white hover:rounded-[12px] active:scale-[0.95] ${className}`}
    >
      {/* Left arrow */}
      <ArrowRight
